...
int main(void)
{
  ...
  unsigned char azmotor1temp;
  unsigned char azmotor2temp;
  unsigned char azmotor3temp;
  unsigned char azmotor4temp;
  unsigned char elmotor1temp;
  unsigned char elmotor2temp;
  unsigned char elmotor3temp;
  unsigned char elmotor4temp;
  ...
  n = recv(sockfd, recvBuff, sizeof(ioStatusResp),0); 
...
/*
  for(i=0;i<8;i++) 
  printf ("servoPLCgeneralStatus bytes :  %d  0x%x \n",i+1,ioStatusResp.ServoPLCgeneralStatus[i]);

  for(i=0;i<12;i++) 
  printf ("servoPLCazimuthStatus bytes :  %d  0x%x \n",i+1,ioStatusResp.ServoPLCazimuthStatus[i]);

  for(i=0;i<12;i++) 
  printf ("servoPLCelevationStatus bytes :  %d  0x%x \n",i+1,ioStatusResp.ServoPLCelevationStatus[i]);
*/

/*
  printf ("servoAmpAz1status: 0x%x\n",ioStatusResp.ServoAmpAz1status);
  printf ("servoAmpAz2status: 0x%x\n",ioStatusResp.ServoAmpAz2status);
  printf ("servoAmpAz3status: 0x%x\n",ioStatusResp.ServoAmpAz3status);
  printf ("servoAmpAz4status: 0x%x\n",ioStatusResp.ServoAmpAz4status);

  printf ("servoAmpEl1status: 0x%x\n",ioStatusResp.ServoAmpEl1status);
  printf ("servoAmpEl2status: 0x%x\n",ioStatusResp.ServoAmpEl2status);
  printf ("servoAmpEl3status: 0x%x\n",ioStatusResp.ServoAmpEl3status);
  printf ("servoAmpEl4status: 0x%x\n",ioStatusResp.ServoAmpEl4status);
*/
  ...
  if (ioStatusResp.ServoAmpAz3status[1] & 1) 
		printf(" Az Amp 3 overvoltage.\n");
  if (ioStatusResp.ServoAmpAz3status[1] & 2) 
		printf(" Az Amp 3 low voltage\n");
  if (ioStatusResp.ServoAmpAz3status[1] & 4) 
		printf(" Az Amp 3 current limit\n");
  if (ioStatusResp.ServoAmpAz3status[1] & 8) 
		printf(" Az Amp 3 motor overload.\n");
  if (ioStatusResp.ServoAmpAz3status[1] & 16) 
		printf(" Az Amp 3 warning.\n");
  if (ioStatusResp.ServoAmpAz3status[1] & 32) 
		printf(" Az Amp 3 error msg available.\n");
  if (ioStatusResp.ServoAmpAz3status[1] & 64) 
		printf(" Az Amp 3 DIGIN2: mode-  manual master\n");
  if (ioStatusResp.ServoAmpAz3status[1] & 128) 
		printf(" Az Amp 3 DIGIN2: mode- computer\n");

  if (ioStatusResp.ServoAmpAz4status[1] & 1) 
		printf(" Az Amp 4 overvoltage.\n");
  if (ioStatusResp.ServoAmpAz4status[1] & 2) 
		printf(" Az Amp 4 low voltage\n");
  if (ioStatusResp.ServoAmpAz4status[1] & 4) 
		printf(" Az Amp 4 current limit\n");
  if (ioStatusResp.ServoAmpAz4status[1] & 8) 
		printf(" Az Amp 4 motor overload.\n");
  if (ioStatusResp.ServoAmpAz4status[1] & 16) 
		printf(" Az Amp 4 warning.\n");
  if (ioStatusResp.ServoAmpAz4status[1] & 32) 
		printf(" Az Amp 4 error msg available.\n");
  if (ioStatusResp.ServoAmpAz4status[1] & 64) 
		printf(" Az Amp 4 DIGIN2: mode-  manual master\n");
  if (ioStatusResp.ServoAmpAz4status[1] & 128) 
		printf(" Az Amp 4 DIGIN2: mode- computer\n");
  ...
  azmotor1temp = (unsigned char) ioStatusResp.ServoAmpAz1status[0]-100;
  azmotor2temp = (unsigned char) ioStatusResp.ServoAmpAz2status[0]-100;
  azmotor3temp = (unsigned char) ioStatusResp.ServoAmpAz3status[0]-100;
  azmotor4temp = (unsigned char) ioStatusResp.ServoAmpAz4status[0]-100;
  elmotor1temp = (unsigned char) ioStatusResp.ServoAmpEl1status[0]-100;
  elmotor2temp = (unsigned char) ioStatusResp.ServoAmpEl2status[0]-100;
  elmotor3temp = (unsigned char) ioStatusResp.ServoAmpEl3status[0]-100;
  elmotor4temp = (unsigned char) ioStatusResp.ServoAmpEl4status[0]-100;

  printf("Az motor temps (C): %d %d %d %d\n",azmotor1temp,azmotor2temp,
                                             azmotor3temp,azmotor4temp);
  printf("El motor temps (C): %d %d %d %d\n",elmotor1temp,elmotor2temp,
                                             elmotor3temp,elmotor4temp);
